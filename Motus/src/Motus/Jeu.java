...
public class Jeu 
{
	...
	public void jouer1(Joueur j,Scanner sc) throws IOException
	{
		if (mode == 1)//mode un joueur
		{
			...
			//déroulement du jeu
			System.out.println("le mot se compose de "+m.getmot().length()+" lettres,Vous avez "+m.nbt()+" tentatives");
			...
			while((!a)&&(nbrt<m.nbt()))
			{
				...
				while(c.length()!=m.getmot().length());
	...
	public void jouer2(Joueur j,Joueur j2,Scanner sc) throws IOException
	{
		...
		if (nbree<2)
		{
			...
			System.out.println("Joueur "+j.getNom()+" votre score reste inchangé et égale à: "+j.getScore());
		}
		else if ((1<nbree)&&(nbree<4))
		{
			j2.CalculScore(3);
